document.addEventListener('DOMContentLoaded', function() {
    ...
    const utils = {
        ...
        // NEW: Get default tab based on user role
        getDefaultTabForUser(user) {
            return user && user.role === 'Manager' ? 'dashboardSection' : 'salesSection';
        },
        // NEW: Check if tab is restricted to managers only
    ...
    const uiManager = {
        ...
        // UPDATED: Show tab with role-based access control
        showTab(tabId) {
        ...
        // UPDATED: Update tab visibility with role-based access control
        updateTabVisibility() {
            ...
            // Always show the menu toggle for all users
            document.getElementById('menuToggle').classList.remove('hidden');
           
            // If current user is not manager and current tab is restricted, redirect to sales
    ...
    // UPDATED: Clock and Sync Management for new layout
    const clockManager = {
    ...
    // UPDATED: Update welcome message based on user role for new layout
    ...
    const loginManager = {
        ...
        // UPDATED: Initialize app with role-based default tab and clock
        initializeApp(user) {
            state.currentUser = user;
            document.getElementById('loginForm').classList.add('hidden');
            document.getElementById('app').classList.remove('hidden');
           
            // Initialize clock and welcome message
            clockManager.init();
            ...
            // Load inventory and setup autocomplete
            inventoryManager.loadInventory(() => {
            ...
            google.script.run
                .withSuccessHandler(response => {
                    ...
                    if (response.success) {
                        ...
                        uiManager.updatePurchaseSummary();
                        inventoryManager.loadInventory(() => {
                            // Clear performance cache when inventory changes
    ...
    const reportsManager = {
        ...
        generateReport(e) {
            ...
            reportsOutput.innerHTML = '<p class="text-center"><i class="fa-solid fa-spinner fa-spin text-primary"></i> Generating report...</p>';
            ...
            try {
                if (reportType === 'daily') {
                    ...
                    // FIXED: Use consistent parameter names for date range
                ...
                google.script.run.withSuccessHandler(response => {
                    try {
                        ...
                        console.error(`Error rendering report: ${error.message}`);
                        ...
                        utils.showAlert(`Error rendering report: ${error.message}`, 'error');
                    }
                }).withFailureHandler(error => {
                    ...
                    utils.showAlert('Error generating report: ' + (error.message || 'Unknown error'), 'error');
                }).generateReport(params);
            } catch (error) {
                console.error(`Error in report parameters: ${error.message}`);
                utils.showAlert('Error in report parameters: ' + error.message, 'error');
        ...
        generateStandardReport(response, reportType) {
            ...
            // Add period display for weekly reports
            let periodHtml = '';
    ...
    const userManager = {
        loadChangePasswordForm() {
            ...
            // User select container visibility is handled by CSS manager-only class
        ...
        loadUsers() {
            ...
            tbody.innerHTML = '<tr><td colspan="2" class="border p-2 text-center"><i class="fa-solid fa-spinner fa-spin text-primary"></i> Loading users...</td></tr>';
    ...
    const exportManager = {
        ...
        exportReport() {
            ...
            try {
                ...
                const headers = Array.from(headerCells).map(cell => {
                    ...
                    // Remove any icons or extra spaces
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    const rowData = Array.from(cells).map(cell => {
                        let text = cell.textContent.trim();
                        // Remove any icons or extra formatting
                        text = text.replace(/<[^>]*>/g, '').trim();
                        return text;
                ...
                utils.showAlert('Error exporting report: ' + (error.message || 'Unknown error'), 'error');
            }
        },
        ...
        downloadCSV(csvContent, filename) {
            try {
                ...
                utils.showAlert('Error downloading file: ' + (error.message || 'Unknown error'), 'error');
            }
        },
    ...
    // Dashboard Management - UPDATED with consistent weekly sales calculation
    const dashboardManager = {
        ...
        // UPDATED: Load dashboard with permission check
        loadDashboard() {
        ...
        // Load Today's Sales calculation to match Reports section logic
        loadTodaySales() {
            const today = utils.formatDateToYYYYMMDD(new Date());
           
            // Use the same logic as Reports section by calling generateReport for daily data
            google.script.run.withSuccessHandler(reportData => {
                if (reportData && reportData.success) {
                    let totalCash = 0;
                    let totalMomo = 0;
                    let transactionCount = 0;
                   
                    // Count transactions exactly like Reports section does
                    if (reportData.sales && Array.isArray(reportData.sales)) {
                        reportData.sales.forEach(sale => {
                            // Only count completed transactions (assuming all sales in the report are completed)
                            const subtotal = (sale.quantity || 0) * (sale.unitPrice || 0);
                            ...
                            // Count each sale as one transaction (same as Reports logic)
                            transactionCount++;
                        });
                    }
                    ...
                    // Fallback to existing method if report fails
                    this.fallbackTodaysSales();
                }
            }).withFailureHandler(error => {
                console.error('Daily report error, falling back:', error);
                this.fallbackTodaysSales();
            }).generateReport({ type: 'daily', date: today });
        },
        ...
        loadStockAlerts() {
            ...
            const fragment = document.createDocumentFragment();
            // Show out of stock items first (highest priority)
            ...
            // Show "more alerts" indicator if needed
        ...
        loadRecentSales() {
            ...
            recentContainer.innerHTML = '<p class="text-center text-secondary py-4"><i class="fas fa-spinner fa-spin mr-2"></i>Loading recent sales...</p>';
        ...
        // FIXED: Load performance summary with consistent weekly calculations
        loadPerformanceSummary() {
            ...
            document.getElementById('topSellingItems').innerHTML = '<p class="text-center text-secondary py-2">Loading top items...</p>';
            console.log('Loading performance summary with consistent weekly calculation...');
            // Get the current week range using the same logic as reports
            const weekRange = utils.getWeekRange(today);
            console.log('Weekly range for performance summary:', weekRange);
            // Use the same report generation logic for consistency
            google.script.run
                .withSuccessHandler(weeklyReport => {
                    console.log('Weekly report response:', weeklyReport);
                   
                    if (weeklyReport && weeklyReport.success) {
                        let weeklyTotal = 0;
                       
                        // Calculate weekly total using the same method as reports section
                        ...
                        console.log('Calculated weekly total:', weeklyTotal);
                       
                        // Update weekly sales with consistent calculation
                        document.getElementById('weeklySales').textContent =
                            `GHC ${weeklyTotal.toFixed(2)}`;
                       
                        // Now get the rest of the performance data
                        this.loadRemainingPerformanceData(weeklyTotal);
                    } else {
                        console.error('Invalid weekly report response:', weeklyReport);
                ...
                .generateReport({
                    type: 'weekly',
                    date: today // This will be converted to week range on server side
                });
        },
        // NEW: Load remaining performance data after getting consistent weekly sales
        loadRemainingPerformanceData(weeklyTotal) {
            const today = utils.formatDateToYYYYMMDD(new Date());
            const month = utils.formatDateToYYYYMM(new Date());
           
            // Get monthly sales using the same report method for consistency
            google.script.run
                .withSuccessHandler(monthlyReport => {
                    let monthlyTotal = 0;
        ...
        // UPDATED: Load top selling items with consistent data source
        loadTopSellingItems() {
            const today = utils.formatDateToYYYYMMDD(new Date());
           
            google.script.run
                .withSuccessHandler(reportData => {
                    const topItems = [];
                   
                    if (reportData && reportData.success && reportData.sales) {
                        // Aggregate sales by item for the week
                        const itemSales = {};
        ...
        // NEW: Update top selling items display
        ...
        // UPDATED: Set default values when data loading fails
        ...
        // Clear cache (call this when inventory data changes significantly)
        clearPerformanceCache() {
            this.performanceCache.data = null;
            this.performanceCache.timestamp = null;
        },
        // Refresh dashboard with cache clearing
        refresh() {
    ...
    const modalManager = {
        setupInventoryModals() {
            ...
            // Add Inventory Form Submit Handler (stays open, resets for next entry)
            utils.safeAddEventListener('addInventoryForm', 'submit', function(e) {
                ...
                google.script.run.withSuccessHandler(response => {
                    ...
                    if (response.success) {
                        ...
                        this.reset();
                        // Restore default purchase date
                        document.getElementById('addPurchaseDate').value = utils.formatDateToYYYYMMDD(new Date());
                        // Focus on first input for quick re-entry
                        document.getElementById('addName').focus();
                        // Refresh inventory table without closing form and clear performance cache
                        inventoryManager.loadInventory(() => {
                            dashboardManager.clearPerformanceCache();
                        });
                    } else {
                    ...
                    submitButton.disabled = false;
                    submitButton.textContent = originalText;
                    utils.showAlert('Error adding item: ' + (error.message || 'Unknown error'), 'error');
                }).addInventoryItem(itemData, state.currentUser.username);
            });
            // Update Inventory Modal with autocomplete
            ...
            // Update Inventory Form Submit Handler (stays open after submission)
            utils.safeAddEventListener('editInventoryForm', 'submit', function(e) {
                ...
                google.script.run.withSuccessHandler(response => {
                    ...
                    if (response.success) {
                        ...
                        // Focus back on name input for next update
                        document.getElementById('editName').focus();
                    } else {
                    ...
                    submitButton.disabled = false;
                    submitButton.textContent = originalText;
                    utils.showAlert('Error updating item: ' + (error.message || 'Unknown error'), 'error');
                }).updateInventoryItem(itemData, state.currentUser.username);
            });
            // Delete Inventory
            utils.safeAddEventListener('deleteInventoryButton', 'click', () => {
                ...
                utils.showConfirmDelete(`Are you sure you want to delete "${state.selectedInventoryItem.name}"?`, () => {
                    google.script.run
                        ...
                        .withFailureHandler(error => {
                            utils.showAlert('Error deleting item: ' + error.message, 'error');
                        })
                        .deleteInventoryItem(state.selectedInventoryItem.name, state.currentUser.username);
                });
            });
            // Bulk Upload Modal
            ...
            utils.safeAddEventListener('bulkUploadForm', 'submit', function(e) {
                ...
                reader.onload = function(e) {
                    ...
                    google.script.run.withSuccessHandler(response => {
                        ...
                        submitButton.disabled = false;
                        submitButton.textContent = originalText;
                        utils.showAlert('Error uploading items: ' + error.message, 'error');
                    }).bulkUploadInventory(items, state.currentUser.username);
                };
            ...
            utils.safeAddEventListener('addUserForm', 'submit', function(e) {
                ...
                google.script.run.withSuccessHandler(response => {
                    ...
                    submitButton.disabled = false;
                    submitButton.textContent = originalText;
                    utils.showAlert('Error adding user: ' + error.message, 'error');
                }).addUser({ username, password, role }, state.currentUser.username);
            });
            utils.safeAddEventListener('deleteUserButton', 'click', () => {
                document.getElementById('deleteUserModal').classList.remove('hidden');
               
                uiManager.setupAutocomplete('deleteUsername', 'deleteUsernameAutocomplete', () => [], (username) => {
                ...
                google.script.run.withSuccessHandler(data => {
                    if (data.success) {
                        ...
                        deleteInput._autocompleteData = usernames;
            ...
            utils.safeAddEventListener('deleteUserForm', 'submit', function(e) {
                ...
                utils.showConfirmDelete(`Are you sure you want to delete user "${username}"? This action cannot be undone.`, () => {
                    ...
                    google.script.run
                        ...
                        .deleteUser(username, state.currentUser.username);
                });
            });
    ...
    function setupEventListeners() {
        ...
        // UPDATED: Dashboard Quick Actions with permission checks
        ...
        utils.safeAddEventListener('changePasswordForm', 'submit', e => {
            ...
            google.script.run.withSuccessHandler(response => {
                ...
                submitButton.disabled = false;
                submitButton.textContent = originalText;
                utils.showAlert('Error changing password: ' + (error.message || 'Unknown error'), 'error');
            }).changePassword(
                state.currentUser ? state.currentUser.username : '',
                targetUsername,
                newPassword,
                isManagerChangingOwnPassword ? oldPassword : '',
