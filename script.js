// script.js - Frontend JavaScript with Fetch API
...
const apiService = {
  async callGoogleAppsScript(action, data = {}, method = 'POST') {
    ...
    for (let attempt = 1; attempt <= CONFIG.retryAttempts; attempt++) {
      try {
        ...
        if (attempt === CONFIG.retryAttempts) {
          throw new Error(`Failed to connect to server after ${CONFIG.retryAttempts} attempts: ${error.message}`);
        }
  ...
  async getInventoryData() {
    return await this.callGoogleAppsScript('getInventory', {}, 'GET');
  },

  async submitSale(saleData) {
    return await this.callGoogleAppsScript('submitSale', { data: saleData });
  },

  async getTodaysSales() {
    return await this.callGoogleAppsScript('getTodaysSales', {}, 'GET');
  },

  async generateReport(params) {
    return await this.callGoogleAppsScript('generateReport', params, 'GET');
  },

  async addInventory(itemData, username) {
    return await this.callGoogleAppsScript('addInventory', { data: itemData, username });
  },

  async updateInventory(itemData, username) {
    return await this.callGoogleAppsScript('updateInventory', { data: itemData, username });
  },
  ...
  async bulkUpload(items, username) {
    return await this.callGoogleAppsScript('bulkUpload', { data: items, username });
  },

  async getUsers() {
    return await this.callGoogleAppsScript('getUsers', {}, 'GET');
  },

  async getUsernames() {
    return await this.callGoogleAppsScript('getUsernames', {}, 'GET');
  },

  async addUser(userData, username) {
    return await this.callGoogleAppsScript('addUser', { data: userData, username });
  },
...
const salesManager = {
  ...
  async submitSale() {
    ...
    try {
      const response = await apiService.submitSale(saleData);
    ...
    try {
      ...
      try {
        ...
        if (reportType === 'summary') {
          tableHtml = this.generateSummaryReport(response);
        } else {
          tableHtml = this.generateStandardReport(response, reportType);
        }
...
const userManager = {
  async loadChangePasswordForm() {
    ...
    if (isManager) {
      try {
        const data = await apiService.getUsernames();
        if (data.success) {
...
const dashboardManager = {
  ...
  async loadTodaySales() {
    ...
    try {
      const reportData = await apiService.generateReport({ type: 'daily', date: today });
      ...
      if (reportData && reportData.success) {
        ...
        await this.fallbackTodaysSales();
      }
    } catch (error) {
  ...
  async fallbackTodaysSales() {
    const today = utils.formatDateToYYYYMMDD(new Date());
    
    try {
      const data = await apiService.getTodaysSales();
  ...
  async loadRecentSales() {
    ...
    try {
      const data = await apiService.generateReport({ type: 'daily', date: today });
  ...
  async loadPerformanceSummary() {
    ...
    try {
      const weeklyReport = await apiService.generateReport({ 
        type: 'weekly', 
        date: today
      });
      ...
      if (weeklyReport && weeklyReport.success) {
        ...
        await this.loadRemainingPerformanceData(weeklyTotal);
      } else {
        console.error('Invalid weekly report response:', weeklyReport);
        await this.loadRemainingPerformanceData(0);
      }
    } catch (error) {
  ...
  async loadRemainingPerformanceData(weeklyTotal) {
    const today = utils.formatDateToYYYYMMDD(new Date());
    const month = utils.formatDateToYYYYMM(new Date());
    
    try {
      const monthlyReport = await apiService.generateReport({ type: 'monthly', month: month });
      let monthlyTotal = 0;
      ...
      await this.loadTopSellingItems();
    } catch (error) {
...
const modalManager = {
  setupInventoryModals() {
    ...
    utils.safeAddEventListener('addInventoryForm', 'submit', async function(e) {
      e.preventDefault();
      ...
      try {
        const response = await apiService.addInventory(itemData, state.currentUser.username);
    ...
    utils.safeAddEventListener('editInventoryForm', 'submit', async function(e) {
      e.preventDefault();
      ...
      try {
        const response = await apiService.updateInventory(itemData, state.currentUser.username);
        submitButton.disabled = false;
        submitButton.textContent = originalText;
    ...
    utils.safeAddEventListener('deleteInventoryButton', 'click', () => {
      ...
      utils.showConfirmDelete(`Are you sure you want to delete "${state.selectedInventoryItem.name}"?`, async () => {
        try {
          const response = await apiService.deleteInventory(state.selectedInventoryItem.name, state.currentUser.username);
          if (response.success) {
            utils.showAlert(response.message, 'success');
            await inventoryManager.loadInventory(() => {
              dashboardManager.clearPerformanceCache();
    ...
    utils.safeAddEventListener('bulkUploadForm', 'submit', async function(e) {
      e.preventDefault();
      ...
      reader.onload = async function(e) {
        ...
        try {
          const response = await apiService.bulkUpload(items, state.currentUser.username);
          submitButton.disabled = false;
          submitButton.textContent = originalText;
          if (response.success) {
            utils.showAlert(response.message, 'success');
    ...
    utils.safeAddEventListener('addUserForm', 'submit', async function(e) {
      e.preventDefault();
      ...
      try {
        const response = await apiService.addUser({ username, password, role }, state.currentUser.username);
        submitButton.disabled = false;
        submitButton.textContent = originalText;
        if (response.success) {
          utils.showAlert(response.message, 'success');
    ...
    utils.safeAddEventListener('deleteUserButton', 'click', () => {
      ...
      apiService.getUsernames().then(data => {
        if (data.success) {
    ...
    utils.safeAddEventListener('deleteUserForm', 'submit', async function(e) {
      e.preventDefault();
      ...
      utils.showConfirmDelete(`Are you sure you want to delete user "${username}"? This action cannot be undone.`, async () => {
        const submitButton = this.querySelector('button[type="submit"]');
        const originalText = submitButton.textContent;
        submitButton.disabled = true;
        ...
        try {
          const response = await apiService.deleteUser(username, state.currentUser.username);
          submitButton.disabled = false;
          submitButton.textContent = originalText;
          if (response.success) {
            utils.showAlert(response.message, 'success');
...
function setupEventListeners() {
  ...
  utils.safeAddEventListener('showSummaryButton', 'click', async () => {
    try {
      const data = await apiService.getTodaysSales();
      if (data && data.success) {
  ...
  utils.safeAddEventListener('refreshInventoryButton', 'click', async () => {
    await inventoryManager.loadInventory(() => {
  ...
  // Change Password
  utils.safeAddEventListener('changePasswordForm', 'submit', async e => {
    e.preventDefault();
